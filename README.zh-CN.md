...
>
> 本仓库在开源 CloudCLI UI 基础上进行二次开发：复用其 React/Node Agent 执行能力，新增 Java 21 + Spring Boot + PostgreSQL Control Plane，把一次不可治理的 Agent 调用升级为可追踪、可审批、可取消、可重试、可归档的正式研发任务。
>
> 正式 AgentTask 链路当前真实接通 Claude Executor。固定四步 Demo 已连续执行两轮，共 **8/8 个真实任务成功**，包含 Edit/Bash 工具暂停审批与恢复，两轮最终测试均为 **3/3 通过**。
>
> **知枢文档入口：** [文档导航](docs/README.md) · [项目总览](docs/zhishu-project.md) · [系统架构](docs/architecture.md) · [仓库结构](docs/repository-layout.md) · [一键演示](docs/demo-runbook.md) · [脱敏证据](docs/demo-evidence.md) · [实施进度](docs/implementation-progress.md)
