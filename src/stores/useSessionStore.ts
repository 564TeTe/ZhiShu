...
function pruneRealtimeSupersededByServer(
  ...
  const reconciledRealtimeMessages = removePersistedAssistantEchoes(serverMessages, realtimeMessages);
...
export function useSessionStore() {
  ...
  /**
   * Like appendRealtime, but replaces an existing message with the same
   * clientMessageId instead of appending a duplicate. Falls back to append
   * when no clientMessageId is provided or no match is found.
   */
  const upsertRealtime = useCallback((sessionId: string, msg: NormalizedMessage, clientMessageId?: string) => {
    const slot = getSlot(sessionId);
    const normalizedMessage =
      msg.sessionId === sessionId ? msg : { ...msg, sessionId };

    if (clientMessageId) {
      const idx = slot.realtimeMessages.findIndex(
        (m) => (m as unknown as Record<string, unknown>)._clientMessageId === clientMessageId,
      );
      if (idx >= 0) {
        // Replace existing — preserve position
