...
function chatMessageToNormalized(
  ...
  return {
    ...
    // Preserve client-side dedup key for upsert
    _clientMessageId: msg._clientMessageId,
  } as NormalizedMessage;
}
...
export function useChatSessionState({
  ...
  const addMessage = useCallback((msg: ChatMessage) => {
    ...
    if (normalized) {
      // Dedup by _clientMessageId: replace existing instead of appending duplicate
      sessionStore.upsertRealtime(activeSessionId, normalized, msg._clientMessageId);
    }
