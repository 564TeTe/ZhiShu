...
const NEW_CHAT_PROVIDERS: LLMProvider[] = ['claude', 'codex', 'opencode'];

const readStoredProvider = (): LLMProvider => {
  const storedProvider = localStorage.getItem('selected-provider');
  // Cursor remains supported for existing sessions, but is intentionally not
  // offered when starting a new chat in the Zhishu portfolio build.
  return NEW_CHAT_PROVIDERS.includes(storedProvider as LLMProvider)
...
export function useChatProviderState({ selectedSession, selectedProject: _selectedProject }: UseChatProviderStateArgs) {
        ...
        PROVIDERS.map(async (p) => {
          try {
            ...
            if (!response.ok || !body.success || !body.data?.models || !body.data?.cache) {
              console.warn(`Unable to load ${p} models: invalid API response.`);
              return null;
            }

            return body.data;
          } catch (error) {
            // Provider CLIs are independent. One unavailable adapter must not
            // erase the successfully loaded catalogs for the other providers.
  ...
  useEffect(() => {
    if (selectedSession || provider !== 'cursor') {
      return;
    }
