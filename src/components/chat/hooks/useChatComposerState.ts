...
import type { ImageProcessingConfig } from '../../../types/vision';

import { useFileMentions } from './useFileMentions';
import { type SlashCommand, useSlashCommands } from './useSlashCommands';
import { submitWithVision } from './useVisionSubmit';
...
export function useChatComposerState({
  ...
  const handleSubmit = useCallback(
    async (
      ...
      // Snapshot everything needed for submission BEFORE clearing state
      const messageContent = currentInput;
      const clientMessageId = Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 9);
      const sendingAttachments = [...currentAttachments];
      const sendingImageFiles = sendingAttachments.filter((f) => f.type.startsWith('image/'));
      const sendingNonImageFiles = sendingAttachments.filter((f) => !f.type.startsWith('image/'));
      ...
      // ---- Async: vision analysis + agent request (does NOT block UI) ----
      await submitWithVision({
        messageContent,
        imageFiles: sendingImageFiles,
        nonImageFiles: sendingNonImageFiles,
        provider,
        ...
        scrollToBottom,
        setIsUserScrolledUp,
        buildSendOptions: (input) => buildSendOptions(input),
        uploadAttachmentFiles,
        selectedSession,
        currentSessionId,
        onSessionEstablished,
        previouslyUploadedAttachments,
        isImageAttachment,
        // Model-level params for vision identity resolution
        executor: (selectedSession?.provider as string) || provider,
        modelId: currentProviderModel,
        modelName: selectedSession?.name || currentProviderModel,
      });
    },
    [
      selectedSession,
