...
export function useProviderAuthStatus(
  ...
  const refreshProviderAuthStatuses = useCallback(async (providers: LLMProvider[] = CLI_PROVIDERS) => {
    ...
    const refreshPromise = Promise.all(
      providers.map((provider) => checkProviderAuthStatus(provider)),
    ).then(() => undefined);
    refreshInFlightRef.current = refreshPromise;

    // Clear only this batch so a StrictMode effect replay reuses the same
    // requests instead of launching another set of provider CLIs.
