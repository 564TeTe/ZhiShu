    ...
    <div className="flex-shrink-0" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0)' }}>
      ...
      {user && (
        <div className="hidden px-2 pb-1.5 md:block">
          <button
            className="flex w-full items-center gap-2 rounded-lg px-2.5 py-1.5 text-muted-foreground transition-colors hover:bg-red-50/60 hover:text-red-600"
            onClick={logout}
          >
      ...
      {user && (
        <div className="px-3 pb-3 pt-2 md:hidden">
          <button
            className="flex h-10 w-full items-center gap-3 rounded-xl bg-muted/40 px-3.5 transition-all hover:bg-red-50/60 active:scale-[0.98]"
            onClick={logout}
          >
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-background/80">
              <LogOut className="h-4 w-4 text-muted-foreground" />
            </div>
            <span className="text-sm font-normal text-foreground">退出登录</span>
          </button>
        </div>
      )}
    </div>
