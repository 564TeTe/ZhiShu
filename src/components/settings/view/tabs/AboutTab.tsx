...
import { ZHISHU_WORDMARK_FONT_FAMILY } from '../../../../constants/branding';
import { useVersionCheck } from '../../../../hooks/useVersionCheck';
import SettingsToggle from '../SettingsToggle';

const GITHUB_REPO_URL = 'https://github.com/564TeTe/ZhiShu';
const DOCS_URL = 'https://github.com/564TeTe/ZhiShu/tree/main/docs';
...
export default function AboutTab() {
  ...
  } = useVersionCheck('564TeTe', 'ZhiShu');
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2">
            <span
              ...
              style={{ fontFamily: ZHISHU_WORDMARK_FONT_FAMILY }}
            >
              知枢
            </span>
            <a
      ...
      <div className="rounded-xl border border-border/50 bg-muted/20 p-4">
        <h3 className="text-sm font-medium text-foreground">知枢项目简介</h3>
        <p className="mt-1 text-xs leading-5 text-muted-foreground">
          统一管理任务、执行、审批、事件和研发产物。普通对话保持轻量，正式研发任务通过控制面形成可追踪、可回放的完整闭环。
        </p>
      </div>
      ...
      <div className="flex items-center justify-between gap-4 border-y border-border/50 py-4">
        <div className="min-w-0">
          <div className="text-sm font-medium text-foreground">
            {t('aboutSettings.updateNotices.label')}
          </div>
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
            {t('aboutSettings.updateNotices.description')}
          </p>
        </div>
        <SettingsToggle
          checked={updateNoticesEnabled}
          onChange={setUpdateNoticesEnabled}
          ariaLabel={t('aboutSettings.updateNotices.label')}
        />
      </div>
