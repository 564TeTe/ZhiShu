...
export default function BrowserUseSettingsTab() {
  ...
  useEffect(() => {
    ...
    void loadSettings()
      .catch((err) => setError(err instanceof Error ? err.message : t('browserUse.messages.loadSettingsFailed')))
      .finally(() => setIsSettingsLoading(false));

    void loadStatus()
      .catch((err) => setError(err instanceof Error ? err.message : t('browserUse.messages.loadStatusFailed')))
  ...
  const updateSettings = async (nextSettings: Partial<BrowserUseSettings>) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('browserUse.messages.saveFailed'));
    } finally {
  ...
  const installBrowserBinaries = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('browserUse.messages.installFailed'));
    } finally {
      setIsStatusLoading(false);
  ...
  const runtimeLabel = (installed?: boolean) => {
    if (isStatusLoading && !status) {
      return t('browserUse.status.checking');
    }
    return installed ? t('browserUse.status.installed') : t('browserUse.status.missing');
  };

  const runtimeDescription = status?.installInProgress
    ? t('browserUse.runtime.installingDescription')
    : !status?.playwrightInstalled && !status?.chromiumInstalled
      ? t('browserUse.runtime.installBoth')
      : status?.playwrightInstalled && !status?.chromiumInstalled
        ? t('browserUse.runtime.installChromium')
        : t('browserUse.runtime.installDescription');
    ...
    <div className="space-y-8">
      ...
      >
        <SettingsCard divided>
          <SettingsRow
            label={t('browserUse.enable.label')}
            description={t('browserUse.enable.description')}
          >
            {isSettingsLoading && !settings ? (
              ...
              <SettingsToggle
                ...
                ariaLabel={t('browserUse.enable.label')}
                disabled={isSaving}
              />
            )}
          </SettingsRow>
          ...
          <div className="space-y-4 px-4 py-4">
            <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
              <span className="rounded-md border border-border px-2 py-1">
                Playwright{t('browserUse.status.separator')} {runtimeLabel(status?.playwrightInstalled)}
              </span>
              <span className="rounded-md border border-border px-2 py-1">
                Chromium{t('browserUse.status.separator')} {runtimeLabel(status?.chromiumInstalled)}
              </span>
              <span className="rounded-md border border-border px-2 py-1">
                {t('browserUse.status.label')}{t('browserUse.status.separator')} {isStatusLoading && !status ? t('browserUse.status.checking') : status?.available ? t('browserUse.status.ready') : browserEnabled ? t('browserUse.status.setupRequired') : t('browserUse.status.disabled')}
              </span>
            </div>
            ...
            {needsBrowserBinaries && (
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0 space-y-1">
                  <div className="text-sm font-medium text-foreground">{t('browserUse.runtime.required')}</div>
                ...
                >
                  ...
                  {isInstalling || status?.installInProgress ? t('browserUse.runtime.installing') : t('browserUse.runtime.install')}
                </Button>
              </div>
            )}
