...
const KNOWN_MAIN_TABS: SettingsMainTab[] = ['agents', 'appearance', 'git', 'api', 'tasks', 'browser', 'notifications', 'plugins', 'vision', 'connections', 'about'];
...
export function useSettingsController({ isOpen, initialTab }: UseSettingsControllerArgs) {
  ...
  const [wallpaperFilename, setWallpaperFilenameState] = useState<string | null>(() => (
    getWallpaperFilename()
  ));
  const [wallpaperSettings, setWallpaperSettingsState] = useState<WallpaperSettings>(() => (
    getWallpaperSettings()
  ));
  ...
  const loadSettings = useCallback(async () => {
    try {
      ...
      setWallpaperFilenameState(getWallpaperFilename());
  ...
  // Wallpaper filename is persisted immediately (localStorage + window event)
  // so the background layer updates without waiting for the debounced autosave,
  // which only handles server-backed preferences.
  const handleWallpaperChange = useCallback((filename: string | null) => {
    setWallpaperFilename(filename);
    setWallpaperFilenameState(filename);
  }, []);

  const handleWallpaperSettingsChange = useCallback((next: WallpaperSettings) => {
    setWallpaperSettings(next);
    setWallpaperSettingsState(next);
  }, []);
