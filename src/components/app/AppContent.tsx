...
function AppContentInner() {
  ...
  // Wallpaper object URL (auth-fetched blob). Null when unset or loading; the
  // layer is skipped entirely then so the default bg-background shows through.
  const { wallpaperUrl, wallpaperSettings } = useWallpaper();
  ...
  return (
    <div
      className={`fixed inset-0 isolate flex bg-background ${wallpaperUrl ? 'app-wallpaper-active' : ''}`}
      style={{
        bottom: 'var(--keyboard-height, 0px)',
        '--wallpaper-surface-opacity': wallpaperSettings.surfaceOpacity,
      } as CSSProperties}
    >
      {wallpaperUrl && (
        <>
          <div
            aria-hidden="true"
            className="pointer-events-none absolute -inset-5 z-0 bg-center bg-no-repeat"
            style={{
              backgroundImage: `url("${wallpaperUrl}")`,
              backgroundSize: wallpaperSettings.fit,
              filter: `blur(${wallpaperSettings.blur}px)`,
              transform: wallpaperSettings.blur > 0 ? 'scale(1.02)' : undefined,
            }}
          />
          <div
            aria-hidden="true"
      ...
      {!isMobile ? (
        <div
          data-sidebar
          className="relative z-10 h-full border-r border-border/50"
          style={{ width: sidebarWidth, minWidth: 0, flexShrink: 0 }}
        >
          <Sidebar {...sidebarSharedProps} />
          {/* Resize drag handle — 6px hot zone */}
          <div
            className="absolute -right-[3px] top-0 z-10 h-full w-[6px] cursor-col-resize transition-colors hover:bg-indigo-500/20 active:bg-indigo-500/30"
        ...
        >
          ...
          />
          <div
            className={`relative z-10 h-full w-[85vw] max-w-sm transform border-r border-border/40 bg-card transition-transform duration-150 ease-out sm:w-80 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
              }`}
