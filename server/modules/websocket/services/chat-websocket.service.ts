...
async function handleChatSend(
  ...
  const baseCommand = typeof data.content === 'string' ? data.content : '';
  // If the frontend sent vision analysis context, prepend it so the agent
  // sees the enriched prompt. The transcript stores the merged command but
  // the frontend displays the original _clientMessageId-identified message.
  const visionContext = typeof clientOptions._visionContext === 'string' && clientOptions._visionContext.trim()
    ? clientOptions._visionContext.trim()
    : '';
  const command = visionContext
    ? `【用户原始问题】\n${baseCommand}\n\n${visionContext}`
    : baseCommand;
