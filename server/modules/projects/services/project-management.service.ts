...
/**
 * Resolves the local project identity consumed by the Task Center Gateway.
 *
 * This is a read-only boundary: it exposes only the stable local id, display
 * name, and absolute project reference needed to resolve Control Plane data.
 */
export function getProjectIdentityById(projectId: string): {
  projectId: string;
  displayName: string;
  projectRef: string;
} | null {
  const projectRow = projectsDb.getProjectById(projectId);
  if (!projectRow || projectRow.isArchived) {
    return null;
  }
  ...
  return {
    projectId: projectRow.project_id,
    displayName: resolveDisplayName(projectRow.custom_project_name, projectRow.project_path),
    projectRef: projectRow.project_path,
  };
}
