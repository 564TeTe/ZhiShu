...
export const runMigrations = (db: Database) => {
  try {
    ...
    db.exec(RUNTIME_COMMAND_DEDUP_TABLE_SCHEMA_SQL);
    db.exec(RUNTIME_EVENT_OUTBOX_TABLE_SCHEMA_SQL);
    db.exec(RUNTIME_CONNECTIONS_TABLE_SCHEMA_SQL);
    const runtimeConnectionColumns = getTableInfo(db, 'runtime_connections').map((column) => column.name);
    addColumnToTableIfNotExists(db, 'runtime_connections', runtimeConnectionColumns, 'last_health_status', 'TEXT');
    addColumnToTableIfNotExists(db, 'runtime_connections', runtimeConnectionColumns, 'last_health_code', 'INTEGER');
    addColumnToTableIfNotExists(db, 'runtime_connections', runtimeConnectionColumns, 'last_health_message', 'TEXT');
    addColumnToTableIfNotExists(db, 'runtime_connections', runtimeConnectionColumns, 'last_checked_at', 'DATETIME');
    addColumnToTableIfNotExists(db, 'runtime_connections', runtimeConnectionColumns, 'last_latency_ms', 'INTEGER');
