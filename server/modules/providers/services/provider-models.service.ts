...
// v4 invalidates catalogs written before the OpenCode verbose-output parser
// was fixed. Those caches could contain the CLI command name (`opencode-models`)
// as a single fake model and would otherwise remain visible for three days.
const PROVIDER_MODELS_CACHE_VERSION = 4;
// Claude and Codex catalogs can change with the active account/provider
// configuration, so stale disk catalogs must never outlive that configuration.
...
export const createProviderModelsService = (dependencies: ProviderModelsServiceDependencies = {}) => {
  ...
  // A custom resolver without an explicit cache path is an injected/test
  // adapter. Keep it memory-only so fake catalogs can never leak into the
  // user's ~/.cloudcli cache. Production and explicit cache tests persist.
  const persistModels = dependencies.resolveProvider === undefined || dependencies.cachePath !== undefined;
