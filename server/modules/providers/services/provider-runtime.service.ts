...
export function createProviderRuntimeService(
  ...
  return {
    ...
    /**
     * Reports whether the provider CLI has usable credentials. Consumed by the
     * formal Runtime start preflight so tasks fail fast with a clear error
     * instead of launching an executor that silently produces nothing.
     */
    async isExecutorAuthenticated(providerName: LLMProvider): Promise<boolean> {
      try {
        const status = await dependencies.resolveProvider(providerName).auth.getStatus();
