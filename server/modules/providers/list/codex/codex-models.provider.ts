...
export class CodexProviderModels implements IProviderModels {
  constructor(private readonly paths: CodexProviderModelPaths = {
    modelsCachePath: CODEX_MODELS_CACHE_PATH,
  ...
  async getSupportedModels(): Promise<ProviderModelsDefinition> {
    try {
      const [raw, configRaw] = await Promise.all([
        readFile(this.paths.modelsCachePath, 'utf8'),
        readFile(this.paths.configPath, 'utf8').catch(() => ''),
      ]);
      ...
      const config = configRaw ? readObjectRecord(TOML.parse(configRaw)) : null;
      const configuredProvider = readOptionalString(config?.model_provider);
      const configuredModel = readOptionalString(config?.model);
      if (configuredProvider && configuredProvider !== 'openai' && configuredModel) {
        const configuredOption = models.find((model) => model.slug === configuredModel);
        return {
          OPTIONS: [configuredOption
            ? mapCodexModel(configuredOption)
            : { value: configuredModel, label: configuredModel }],
