 ...
 * @param {string} permissionMode - 'plan', 'default', 'acceptEdits', or 'bypassPermissions'
 * @returns {object} - { sandboxMode, approvalPolicy }
 */
function mapPermissionModeToCodexOptions(permissionMode) {
  switch (permissionMode) {
    case 'plan':
      // Formal read-only tasks: the SDK may only read the workspace and never
      // prompts for approvals.
      return {
...
export async function queryCodex(command, options = {}, ws, context) {
  ...
  try {
    // Strip CODEX_API_KEY so Codex uses ChatGPT OAuth tokens instead of
    // an API key that may have no credits — the user's Plus subscription
    // is tied to their OAuth identity, not to a standalone API key.
    const codexEnv = {};
    for (const key of Object.keys(process.env)) {
      if (key !== 'CODEX_API_KEY') codexEnv[key] = process.env[key];
    }
