  ...
  return new Promise((resolve, reject) => {
    ...
    void context.resolveResumeModel(sessionId, model).then(async (resolvedModel) => {
      ...
      if ((command && command.trim()) || hasAttachments) {
        ...
        // Keep the prompt off the Windows .cmd argument boundary. Writing
        // UTF-8 directly to stdin preserves Chinese text and multiline task
        // context without depending on the active console code page.
        promptInput = appendFilesInputTag(
      ...
      opencodeProcess = spawnFunction('opencode', args, {
        ...
        // The server runs without a console; without this Windows allocates a
        // visible console window for the CLI that users may close (killing the
        // run with STATUS_CONTROL_C_EXIT).
        windowsHide: true,
      });

      activeOpenCodeProcesses.set(processKey, opencodeProcess);
      opencodeProcess.sessionId = processKey;
      opencodeProcess.stdin.end(promptInput, 'utf8');
