...
test('spawnOpenCode emits session_created before normalized live messages for new sessions', async () => {
  ...
  try {
    ...
    const chinesePrompt = '读取当前项目并回答：一加一等于几？';
    await opencodeRuntime.run(chinesePrompt, { cwd: tempRoot }, writer, runtimeContext);
    ...
    assert.equal(capture.stdin, chinesePrompt);
    assert.equal(launchedArgs.includes(chinesePrompt), false);
    ...
    const attachmentPrompt = attachmentOnlyCapture.stdin;
