...
export class ClaudeProviderAuth implements IProviderAuth {
  ...
  private checkInstalled(): boolean {
    ...
    try {
      spawn.sync(cliPath, ['--version'], { stdio: 'ignore', timeout: 5000, windowsHide: true });
      return true;
    } catch {
      return false;
    }
  ...
  private async loadSettingsEnv(): Promise<Record<string, unknown>> {
    try {
      const settingsPath = path.join(this.resolveUserHome(), '.claude', 'settings.json');
  ...
  private async checkCredentials(): Promise<ClaudeCredentialsStatus> {
    ...
    try {
      const credPath = path.join(this.resolveUserHome(), '.claude', '.credentials.json');
  ...
  private resolveUserHome(): string {
