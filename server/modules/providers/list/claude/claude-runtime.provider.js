...
const pendingAbortSessionIds = new Set();
...
function mapCliOptionsToSDK(options = {}) {
  const { providerSessionId, cwd, toolsSettings, permissionMode, effort, settingSources, settings: inlineSettings, env } = options;
  ...
  sdkOptions.settingSources = Array.isArray(settingSources)
    ? settingSources
    : ['project', 'user', 'local'];
  if (inlineSettings) {
    sdkOptions.settings = inlineSettings;
  }
...
async function queryClaudeSDK(command, options = {}, ws, context) {
  const { sessionId, sessionSummary, runtimeSessionKey } = options;
  ...
  const sessionKey = () => runtimeSessionKey || sessionId || capturedSessionId || null;
  ...
  try {
    ...
    sdkOptions.canUseTool = async (toolName, input, context) => {
      ...
      const decision = await waitForToolApproval(requestId, {
        ...
        metadata: {
          ...
          _sessionId: runtimeSessionKey || sessionId || capturedSessionId || null,
          _toolName: toolName,
          _input: input,
    ...
    if (sessionKey()) {
      ...
      if (pendingAbortSessionIds.delete(sessionKey())) {
        abortedSessionIds.add(sessionKey());
...
async function abortClaudeSDKSession(sessionId) {
  ...
  if (!session) {
    // Start is accepted before the SDK query constructor has necessarily
    // registered the active session. Remember the cancellation and interrupt
    // immediately after registration instead of losing the command.
    pendingAbortSessionIds.add(sessionId);
    console.log(`Session ${sessionId} not active yet; cancellation queued`);
    return true;
  }
