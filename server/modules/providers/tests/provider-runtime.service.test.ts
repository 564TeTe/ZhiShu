...
test('isExecutorAuthenticated reflects the provider auth status', async () => {
  const runtime = createRuntime();
  const provider = createProvider('codex', runtime);
  const service = createService([provider]);

  assert.equal(await service.isExecutorAuthenticated('codex'), true);
  ...
  const unauthenticatedService = createService([unauthenticatedProvider]);
  assert.equal(await unauthenticatedService.isExecutorAuthenticated('opencode'), false);

  const brokenAuthService = createService([{
    ...createProvider('cursor', runtime),
    auth: {
      async getStatus() {
  ...
  assert.equal(await brokenAuthService.isExecutorAuthenticated('cursor'), false);
});
