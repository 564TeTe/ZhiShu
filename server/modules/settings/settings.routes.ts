...
export function createSettingsRouter(
  ...
  router.get('/vision/mode', respond((req) => service.getImageProcessingMode(
    userId(req), queryString(req.query.sessionId),
  )));
  router.put('/vision/mode', respond((req) => service.setImageProcessingMode(
    userId(req), req.body?.mode, queryString(req.body?.sessionId),
  )));

  // Get vision status with optional model identity resolution.
  // Pass model-level params as query string: executor, modelId, modelName, modelAlias, sessionId.
  router.get('/vision/status', respond((req) => service.getVisionStatus(
    userId(req),
    queryString(req.query.provider),
    {
      executor: queryString(req.query.executor),
      modelId: queryString(req.query.modelId),
      modelName: queryString(req.query.modelName),
      modelAlias: queryString(req.query.modelAlias),
      sessionId: queryString(req.query.sessionId),
    },
  )));

  // Session-level DeepSeek image assist toggle
  router.put('/vision/deepseek-assist', respond((req) => service.setDeepSeekImageAssist(
    userId(req),
    queryString(req.body?.sessionId) || queryString(req.query?.sessionId) || '',
    req.body?.value || req.body?.deepseekImageAssist,
  )));
