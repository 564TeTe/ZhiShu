...
// Wallpapers can be large (4K images), so they get a higher size cap and a
// single-file limit. They share the same on-disk store and serving route as
// chat images so no separate read path is needed.
...
/**
 * Stores one user-selected wallpaper image in the global `~/.cloudcli/assets`
 * folder. Returns the stored filename so the client can fetch it later through
 * GET /api/assets/images/:filename and apply it as the app background.
 */
router.post('/wallpaper', (req, res) => {
  wallpaperUpload.single('wallpaper')(req, res, (err: unknown) => {
    if (err) {
      const message = err instanceof Error ? err.message : 'Upload failed';
      return res.status(400).json({ error: message });
    }
