...
// ---------------------------
//----------------- ZHISHU RUNTIME PROTOCOL ------------
/**
 * Capability names accepted by the version 1 Zhishu Runtime Protocol.
 *
 * The Control Plane sends these provider-neutral values. The Runtime module
 * uses them to describe and enforce the tools available to one Agent attempt;
 * provider-specific tool names must never leak into the Java domain model.
 */
...
/**
 * Permission policy frozen into one Runtime start command.
 *
 * Entries identify capabilities that require a Control Plane decision before
 * the corresponding provider tool may continue. This is behavior context in
 * the first slice; the provider permission hook remains the enforcement point.
 */
export type RuntimePermissionPolicy = {
  requireApprovalFor: RuntimeCapability[];
  mode: 'MANUAL' | 'AUTO_SAFE' | 'AUTO_TRUSTED';
  projectRoot: string;
...
/**
 * One durable output from the parent Attempt supplied to a follow-up run.
 *
 * The Control Plane includes only bounded summary, file-change, and test-report
 * artifacts. Runtime renders each category separately so historical output is
 * treated as evidence rather than as unstructured instructions.
 */
export type RuntimeFollowUpEvidence = {
  type: 'SUMMARY' | 'FILE_CHANGE' | 'TEST_REPORT' | 'ERROR_REPORT' | string;
  content?: string | null;
  metadata?: Record<string, unknown>;
};

export type RuntimeFollowUpAttempt = {
  attemptId: string;
  ...
  evidence: RuntimeFollowUpEvidence[];
};

/**
 * Frozen follow-up context for a new Attempt under the same task ID.
 *
 * It links the parent Attempt and preserves the original objective, prior
 * outcome, user correction, and new acceptance requirement without relying on
 * provider chat-session continuity.
 */
export type RuntimeTaskFollowUpContext = {
  ...
  attemptHistory?: RuntimeFollowUpAttempt[];
...
/**
 * Version 1 command sent by the Spring Control Plane to start one real Agent attempt.
 *
 * `idempotencyKey` remains stable across HTTP retries, while `requestId` identifies
 * an individual transport attempt. `projectRef` is a Node-local absolute workspace
 * path in the single-machine Golden Core deployment.
 */
export type RuntimeStartCommand = {
  ...
  context?: {
    ...
    vision?: string | null;
    attachments?: unknown[];
    taskFollowUpContext?: RuntimeTaskFollowUpContext;
    workOrderContext?: RuntimeWorkOrderContextV1;
  };
};

/** Immutable, versioned Plan-to-Runtime context captured by the Execution Bridge. */
export type RuntimeWorkOrderContextV1 = {
  schemaVersion: '1.0';
  workOrderId: string;
  planId: string;
...
/**
 * Stable acceptance snapshot persisted by Node for one idempotent Start command.
 *
 * Replays with the same key and semantic request body return this exact response
 * without launching another provider process.
 */
export type RuntimeAcceptedResponse = {
  runtimeRunId: string;
  accepted: true;
};

/**
 * Version 1 command used by the Control Plane to cancel one active Runtime run.
 *
 * The idempotency key remains stable across HTTP retries. Node records the
 * semantic command before invoking the provider abort operation, so an exact
 * replay returns the first response without repeating the side effect.
 */
export type RuntimeCancelCommand = {
  protocolVersion: '1.0';
  requestId: string;
  idempotencyKey: string;
};
...
/**
 * Version 1 command used to resolve one pending provider permission request.
 *
 * The Runtime accepts only APPROVED or DENIED decisions. A stable idempotency
 * key prevents a transport retry from resolving the same pending promise twice.
 */
export type RuntimeApprovalDecisionCommand = {
  protocolVersion: '1.0';
  requestId: string;
  idempotencyKey: string;
  decision: 'APPROVED' | 'DENIED';
  message?: string;
  decisionSource?: 'MANUAL' | 'AUTO_POLICY' | 'POLICY_DENY' | 'TIMEOUT' | string;
  policy?: string;
  rule?: string;
  reason?: string;
};
...
/**
 * Strong business event names emitted from the Node Runtime to the Control Plane.
 *
 * Token and thinking deltas are intentionally excluded; only events useful for
 * durable task state, approvals, diagnostics, and final artifacts cross planes.
 */
...
/** Durable user-facing outputs projected by the Control Plane. */
...
/**
 * Versioned event envelope reported from Node to Spring for one Task Attempt.
 *
 * `eventId` supports receiver idempotency and `sequenceNo` is monotonically
 * increasing within a Runtime run. The payload must never contain credentials.
 */
...
/**
 * Event batch accepted by the Spring Control Plane internal callback endpoint.
 *
 * Batching is part of the protocol from version 1 even though the initial Node
 * implementation normally reports one event at a time.
 */
...
/**
 * One undelivered Runtime event stored in the Node SQLite outbox.
 *
 * The database module persists this shape and the Runtime reporter consumes it
 * when selecting the next event. `attempts` counts failed HTTP deliveries;
 * `nextAttemptAt` is an epoch-millisecond retry deadline. Only the lowest
 * pending `sequenceNo` for a Runtime run may be returned for delivery.
 */
export type RuntimeEventOutboxEntry = {
  event: RuntimeEvent;
  attempts: number;
...
/**
 * Aggregate SQLite outbox statistics used by Runtime delivery health checks.
 *
 * Timestamps are epoch milliseconds. `oldestCreatedAt` is null for an empty
 * queue, while `lastError` contains only a diagnostic message and must never
 * include credentials or event payloads.
 */
...
/**
 * Public, secret-free health snapshot for Node-to-Spring event delivery.
 *
 * `misconfigured` means new formal Agent runs must be rejected. `degraded`
 * means configuration is valid but at least one event has failed delivery.
 * A non-empty queue with zero failures remains `up` because normal asynchronous
 * delivery can briefly leave newly committed events pending.
 */
export type RuntimeEventDeliveryHealth = {
...
/**
 * Runtime lifecycle snapshot exposed to the internal cancel and approval paths.
 *
 * This state is process-local execution state, not the durable business truth;
 * the Spring TaskAttempt state machine remains authoritative across restarts.
 */
export type RuntimeRunSnapshot = {
  ...
  attemptId: string;
  executor: LLMProvider;
  status: 'accepted' | 'running' | 'waiting_approval' | 'succeeded' | 'failed' | 'aborted';
  sequenceNo: number;
  startedAt: string;
...
/**
 * Durable Node-side record for a side-effecting Runtime command.
 *
 * The Runtime module compares `requestHash` on retries and replays
 * `responseJson` only when both the idempotency key and semantic payload match.
 */
export type RuntimeCommandDedupRecord = {
  idempotencyKey: string;
...
// ---------------------------
//----------------- STATE STEWARD REPORT CONTRACT ------------

/**
 * Semantic Project State entry kinds that State Steward may propose.
 * Project Intelligence analyzes these candidates and Task Center Gateway
 * validates the same closed set before crossing the Project Control boundary.
 * FACT is deliberately excluded because only verification reducers may create it.
 */
export type StateStewardEntryType = 'DISCOVERY' | 'DECISION' | 'CONSTRAINT' | 'RISK';

/**
 * One non-authoritative State candidate carried by StructuredExecutionReportV1.
 * Project Intelligence may merge equivalent candidates; Project Control validates
 * referenced evidence again before a user-confirmed State entry is persisted.
 */
export type StateStewardCandidate = {
  entryType: StateStewardEntryType;
  title: string;
  content: string;
...
/**
 * Versioned execution summary accepted by State Steward through Task Center Gateway.
 * This report can produce only a State Change Proposal and is never itself treated as
 * verified fact. Project Intelligence and Task Center Gateway share this transport shape.
 */
export type StructuredExecutionReportV1 = {
  reportId: string;
  reportVersion: '1.0';
  result: 'SUCCESS' | 'PARTIAL' | 'BLOCKED' | 'FAILED';
  discoveries?: StateStewardCandidate[];
  decisions?: StateStewardCandidate[];
  constraints?: StateStewardCandidate[];
  risks?: StateStewardCandidate[];
};

/**
 * Minimal authoritative State entry read by the deterministic State Steward analyzer.
 * Task Center Gateway projects Control Plane snapshots into this shape before invoking
 * Project Intelligence; non-Steward entry types are filtered out by the caller.
 */
export type StateStewardStateEntry = {
  entryId: string;
  entryType: StateStewardEntryType;
  title: string;
  content: string;
