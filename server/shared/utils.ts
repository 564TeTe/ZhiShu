...
/**
 * Finds the repository root visible from a starting directory.
 *
 * Provider skill systems such as Codex and OpenCode walk upward through parent
 * folders when resolving repository/project skills. The first marker is the
 * repository boundary for this workspace; an unrelated parent checkout must
 * not change repository skills into user or project sources.
 */
export async function findTopmostGitRoot(startPath: string): Promise<string | null> {
  ...
  while (true) {
    if (await hasGitMarker(currentPath)) {
      // A nested repository is its own provider boundary. Do not continue into
      // an unrelated parent repository (for example the user's home checkout).
      return currentPath;
    }
