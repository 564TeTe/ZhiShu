...
import { runtimeConnectionRoutes } from './modules/runtime-connections/index.js';
...
import { taskCenterGatewayRoutes } from './modules/task-center-gateway/index.js';
import { worktreesRoutes } from './modules/worktrees/index.js';
import browserUseMcpRoutes from './modules/browser-use/browser-use-mcp.routes.js';
import { initializeVisionModule, visionRoutes } from './modules/vision/index.js';
import {
    getRuntimeEventDeliveryHealth,
    runtimeRoutes,
    startRuntimeEventDelivery,
    stopRuntimeEventDelivery,
...
app.get('/health', (req, res) => {
    res.json({
        ...
        runtimeEventDelivery: getRuntimeEventDeliveryHealth(),
    });
});

// Versioned Control Plane -> Execution Plane commands. This internal surface
// uses its own shared-secret authentication and never depends on browser auth.
app.use('/internal/runtime/v1', runtimeRoutes);
...
// New Task Center read gateway (protected; browser never talks to Control Plane directly)
app.use('/api/task-center', authenticateToken, taskCenterGatewayRoutes);
...
app.use('/api/runtime-connections', authenticateToken, runtimeConnectionRoutes);
...
async function startServer() {
    try {
        ...
        // Initialize vision module (register + activate providers)
        ...
        const shutdownRuntimeServices = async () => {
            try {
                await stopRuntimeEventDelivery();
            } catch (err) {
                console.error('[Runtime Outbox] Error stopping delivery during shutdown:', getErrorMessage(err));
            }
            try {
