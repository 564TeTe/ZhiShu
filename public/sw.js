// Service Worker for Zhishu PWA
// Cache only manifest (needed for PWA install). HTML and JS are never pre-cached
// so a rebuild + refresh always picks up the latest assets.
const CACHE_NAME = 'zhishu-ui-v4';
const urlsToCache = [
  '/manifest.json'
];
...
self.addEventListener('push', event => {
  ...
  try {
    ...
    payload = { title: '知枢', body: event.data.text() };
  }
  ...
  event.waitUntil(
    self.registration.showNotification(payload.title || '知枢', options)
  );
});
